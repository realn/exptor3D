...
/*	USTAWIENIE WSKANIKA
	By wogóle zainicjalizowaæ okno i kontekst
	musimy mieæ uchwyt na okno. Mog³em go wkodowaæ
	ju¿ w klasie, ale to utrudni niektóre rzeczy,
	dlatego postanowi³em ustawiæ tylko wskaŸnik.
*/
void UIRender::SetWinHandle( HWND* Uchwyt )
{
	if( IsWin ) return; // Je¿eli okno jest stworzone, to stracenie wskaŸnika
						// by³o by katastrof¹! Dlatego przerywamy metodê.

	Log.Log( "Ustawianie uchwytu Okna" );
	hWnd = Uchwyt; // Ustawiamy uchwyt, przez przekazanie adresu ( najproœciej )
}
